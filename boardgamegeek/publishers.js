...
//xml sel -T -t -m "/boardgames/boardgame[@objectid='12333']" -v "concat(./boardgamepublisher[1],',',./boardgamepublisher[2],',',./boardgamepublisher[3],',',./boardgamepublisher[4])" top200.xml

(function(){
  //var base="xml sel -t -v \"/boardgames/boardgame[@objectid=\'__ID__\']/boardgamepublisher\" top200.xml >> TOPpublishers.txt";
  var base="xml sel -n -T -t -m \"/boardgames/boardgame[@objectid='__ID__']\" -v \"concat(./boardgamepublisher[1],',',./boardgamepublisher[2],',',./boardgamepublisher[3],',',./boardgamepublisher[4])\" top200.xml >>TOPpublishers.txt";
  var ID=ProcFile("C:\\boardgamegeek\\toprankedIDs.txt");
  for(var i=0;i<ID.length;i++){
    WScript.Echo(base.replace("__ID__",ID[i]));
    WScript.Echo("echo. >> TOPpublishers.txt"); // delimiter
  }
})();